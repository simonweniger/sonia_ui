...
async function checkAllComponents(
  ...
  logger.break()
  logger.info(
    `Run ${highlighter.success(`sonia diff <component>`)} to see the changes.`,
  )
  logger.info(
    `Run ${highlighter.success(`sonia add ${componentsWithUpdates.map((component) => component.name).join(' ')} --overwrite`)} to update the component(s).`,
  )
}

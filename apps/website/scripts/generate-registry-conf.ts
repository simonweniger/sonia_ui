...
function isIconDependency(_import: string) {
  ...
  if (
    _import.includes('/icons') &&
    (_import.endsWith('/index.ts') || _import.endsWith('/icons'))
  ) {
    return true
  }
  return false
...
function extractIconsFromBarrelImport(
  content: string,
  ...
  return namedImports
    ...
    .map((name) => {
      ...
      return (
        withoutIcon.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase() + '-icon'
      )
    })
}
...
async function generateIconsRegistry(dir: string) {
  const { icons, iconLibraries } = await import(join(dir, 'registry-icons.ts'))
